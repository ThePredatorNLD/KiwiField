...
public class KiwiListener implements Listener {
		...
		if(event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK){
			...
			if(player.getItemInHand().getItemMeta().hasLore()){
				if(player.getItemInHand().getItemMeta().getLore().contains(ChatColor.GRAY+"MP5")){
					...
					int speed = 10;
					proj.setVelocity(new Vector(vec.getX() * speed * 3.5, vec.getY() * speed * 3.5, vec.getZ() * speed * 3.5));
				...
				if(player.getItemInHand().hasItemMeta()){
				if(player.getItemInHand().getItemMeta().hasLore()){
				if(player.getItemInHand().getItemMeta().getLore().contains(ChatColor.GRAY+"RPG-7V2")){
					if(!waitMapD.containsKey(player.getName())){
						...
						Vector vec = player.getLocation().getDirection(); 
						int speed = 5;
						proj.setVelocity(new Vector(vec.getX() * speed * 3.5, vec.getY() * speed * 4, vec.getZ() * speed * 3.5));
						proj.setVelocity(proj.getVelocity().multiply(0.1));
	...
	public void onProjectileHit(ProjectileHitEvent event) {
		if (event.getEntity() instanceof org.bukkit.entity.Snowball) {
			...
			if(snowballH.hasMetadata("granade")){
				...
				final Item grsno = snowballH.getWorld().dropItem(snowballH.getLocation(), new ItemStack(Material.CLAY_BALL, 1));
					...
					for(int i=0; i<entitys.size();i++){
						...
						if(entity instanceof Player){
							...
							if(((Damageable) entity).getHealth()<=15 && entity!=proj.getShooter()){
								...
								//Sou läuft das!
								if(uwMG.containsKey(player.getName())){uwMG.remove(player.getName());}
								if(uwHG.containsKey(player.getName())){uwHG.remove(player.getName());}
								if(uwSN.containsKey(player.getName())){uwSN.remove(player.getName());}
								if(uwRL.containsKey(player.getName())){uwRL.remove(player.getName());}
					...
					for(int i=0; i<entitys.size();i++){
						...
						if(entity instanceof Player){
							...
							if(((Damageable) entity).getHealth()<=6 && entity!=proj.getShooter()){
								...
								if(uwHG.containsKey(player.getName())){uwHG.remove(player.getName());}
								if(uwSN.containsKey(player.getName())){uwSN.remove(player.getName());}
								if(uwGR.containsKey(player.getName())){uwGR.remove(player.getName());}
	...
	public void onPlayerPickUp(PlayerPickupItemEvent event) {
		if(event.getItem().hasMetadata("ubrot") || event.getItem().hasMetadata("medikit") || event.getItem().hasMetadata("ammokit")){
			event.setCancelled(true);
		}
	}
	...
	public void onPlayerThrowMedikit(PlayerDropItemEvent event) {
					...
					if(event.getItemDrop().getItemStack().getType()==Material.SLIME_BALL){event.setCancelled(true);}
					if(event.getItemDrop().getItemStack().getType()==Material.CLAY_BALL){event.setCancelled(true);}
					if(event.getItemDrop().getItemStack().getType()==Material.FLINT){event.setCancelled(true);}
					if(event.getItemDrop().getItemStack().getType()==Material.BLAZE_ROD){event.setCancelled(true);}
					if(event.getItemDrop().getItemStack().getType()==Material.BONE){event.setCancelled(true);}
	...
	if(!waitMapE.containsKey(event.getPlayer().getName())){	
		...
		plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {public void run() {medikit.remove();waitMapE.remove(playerM.getName());}}, 100L);
	...
	if(!waitMapF.containsKey(event.getPlayer().getName())){	
		...
		for(int i=0; i<entitys.size();i++){
			...
			if(entity instanceof Player){
				...
				if(player.getInventory().getLeggings()!=null){player.getInventory().getLeggings().setDurability((short) 0);}
		...
		plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new Runnable() {public void run() {medikit.remove();waitMapF.remove(playerM.getName());}}, 100L);
	...
	public void onPlayerDeath(PlayerDeathEvent event) {
		...
		if(event.getDeathMessage().contains("fell out of the world")){KillFail=true;event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getName()+ChatColor.WHITE+" fiel über Bord");}
		if(event.getDeathMessage().contains("died")){KillFail=true;event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getName()+ChatColor.WHITE+" [SELFKILL]");}
		if(event.getDeathMessage().contains("using Machine Gun")){KillFail=true;event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getKiller().getName()+ChatColor.WHITE+" [MP5] "+ChatColor.RED+event.getEntity().getDisplayName());}
		if(event.getDeathMessage().contains("using Rocket Launcher")){KillFail=true;event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getKiller().getName()+ChatColor.WHITE+" [RPG-7V2] "+ChatColor.RED+event.getEntity().getDisplayName());}
		if(event.getDeathMessage().contains("using Handgun")){KillFail=true;event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getKiller().getName()+ChatColor.WHITE+" [M1911] "+ChatColor.RED+event.getEntity().getDisplayName());}
		if(event.getDeathMessage().contains("using Sniper")){KillFail=true;event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getKiller().getName()+ChatColor.WHITE+" [M98B] "+ChatColor.RED+event.getEntity().getDisplayName());}
		if(event.getDeathMessage().contains("using Granade")){KillFail=true;event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getKiller().getName()+ChatColor.WHITE+" [M67 Granade] "+ChatColor.RED+event.getEntity().getDisplayName());}
		
		if(KillFail==false){
		String heads = "";
		if(headshot.containsKey(event.getEntity().getName())){heads = "<>";}
		
		if(uwMG.containsKey(event.getEntity().getName())){
		event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getKiller().getName()+ChatColor.WHITE+" [MP5] "+heads+" "+ChatColor.RED+event.getEntity().getDisplayName());
		uwMG.remove(event.getEntity().getName());
		if(headshot.containsKey(event.getEntity().getName())){headshot.remove(event.getEntity().getName());}
		}
		
		if(uwHG.containsKey(event.getEntity().getName())){
		event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getKiller().getName()+ChatColor.WHITE+" [M1911] "+heads+" "+ChatColor.RED+event.getEntity().getDisplayName());
		uwHG.remove(event.getEntity().getName());
		if(headshot.containsKey(event.getEntity().getName())){headshot.remove(event.getEntity().getName());}
		}
		
		if(uwSN.containsKey(event.getEntity().getName())){
		event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getKiller().getName()+ChatColor.WHITE+" [M98B] "+heads+" "+ChatColor.RED+event.getEntity().getDisplayName());
		uwSN.remove(event.getEntity().getName());
		if(headshot.containsKey(event.getEntity().getName())){headshot.remove(event.getEntity().getName());}
		}
		
		if(uwGR.containsKey(event.getEntity().getName())){
		event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getKiller().getName()+ChatColor.WHITE+" [M67 Granade] "+heads+" "+ChatColor.RED+event.getEntity().getDisplayName());
		uwGR.remove(event.getEntity().getName());
		}
		
		if(uwRL.containsKey(event.getEntity().getName())){
		event.setDeathMessage(""+ChatColor.GREEN+event.getEntity().getKiller().getName()+ChatColor.WHITE+" [RPG-7V2] "+heads+" "+ChatColor.RED+event.getEntity().getDisplayName());
		...
		if(event.getDamager() instanceof org.bukkit.entity.Snowball) {
			...
			if(event.getDamager().hasMetadata("mg")){
				...
				if(((Damageable) event.getEntity()).getHealth()<=6 && event.getEntity()!=proj.getShooter()){
					...
					if(proj.getLocation().getY() - event.getEntity().getLocation().getY() > 1.35d){headshot.put(((Player) event.getEntity()).getName(), 1);}
				}else{
